package org.graphics
{
	...
	public class B2Graphics extends Sprite
	{
		...
		public function B2Graphics(w:Number,h:Number)
		{
			super();
		...
		public function line( fx:Number, fy:Number, tx:Number, ty:Number, color:uint = 0, alpha:Number = 1, thick:Number = 1):void{
			_shape_layer.graphics.lineStyle( thick, color, alpha, true,"normal" );
			_shape_layer.graphics.moveTo( fx, fy );
			_shape_layer.graphics.lineTo( tx, ty );
		}
		
		public function rect( x:Number, y:Number, w:Number, h:Number, color:uint = 0, fill:uint = 1, alpha:Number= 1, thick:Number =1 ):void{
			if(fill == 1)
		...
		public function circle( ox:Number, oy:Number, r:Number, color:uint = 0, fill:uint =  1, alpha:Number = 1, thick:Number = 1 ):void{
			if(fill == 1)
				_shape_layer.graphics.beginFill( color, alpha );
		...
		public function polo( points:String, color:uint = 0, fill:uint = 1, alpha:Number = 1, thick:Number = 1):void{
			var decoder:JSONDecoder = new JSONDecoder(points,true);
			var pl:Array = decoder.getValue();
			if(fill == 1)
				_shape_layer.graphics.beginFill( color, alpha );
				_shape_layer.graphics.lineStyle( thick, color, thick==0?0:alpha );
		...
		public function wedge( x:Number, y:Number, ir:Number, or:Number, aStart:Number, aEnd:Number, color:uint= 0, fill:uint = 1, alpha:Number = 1, thick:Number = 1, step:Number = 1 ):void{
			if(fill == 1)
				_shape_layer.graphics.beginFill( color, alpha );
				_shape_layer.graphics.lineStyle( thick, color, alpha );
		...
		public function text( x:Number, y:Number, text:String,color:Number = 0, align:String = "left", fontSize:uint = 12, bold:uint = 1, background:Object = null, rotation:Number = 0, max_width:Number = 0 ):void{
			var myFormat:TextFormat = new TextFormat();
			myFormat.align = align;
			myFormat.font = "arial";
			myFormat.size = fontSize;
			myFormat.color = color;
			myFormat.bold = bold == 1 ? true : false;
